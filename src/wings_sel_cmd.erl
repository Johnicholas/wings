...
command({edge_loop,{nth_edge_ring,true}}, St) ->
    select_nth_ring(St);
command({edge_loop,{nth_edge_ring,N}}, St) ->
    Nth = case N of [Num] -> Num; N -> N end,
    {save_state,wings_edge:select_nth_ring(Nth,St)};
...
ask(Qs, Fun, St) ->
    ...
    fun
        ({dialog_preview,Res}) ->
            Sel = Fun(Res),
            {preview,{select,{by,{id,Sel}}},St};
        (cancel) -> St;
        (Res) ->
            Sel = Fun(Res),
            {commit,{select,{by,{id,Sel}}},St}
    end).
